...
async function updatecourses(profileData) {
    ...
    const res = await axios.put("http://localhost:7007/profile-update/" + instituteId, profileData)
    return res;
}

async function courseList(profileData) {
    const instituteId = localStorage.getItem('institute_id');
    const result = await axios.get("http://localhost:7007/profile-List/" + instituteId, profileData)
    return result;
}

async function EditCourse(profileData) {
     const instituteId = localStorage.getItem('institute_id');
    const result = await axios.put("http://localhost:7007/profile-update/" +  instituteId, profileData)
    return result;
}
